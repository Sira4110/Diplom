class RSAEncryptor {
    constructor() {
        ...
        // Повідомлення для підтримки перекладу
        this.messages = {
            en: {
                generatingKeys: "Generating keys, please wait... (this may take a while)",
                keyGenError: "Error during key generation. Check the console for details.",
                enterTextEncrypt: "Please enter text to encrypt!",
                enterPublicKey: "Please enter a public key for encryption!",
                encryptionError: "Error during encryption. Check the console for details.",
                enterEncryptedText: "Please enter encrypted text to decrypt!",
                enterPrivateKey: "Please enter a private key for decryption!",
                decryptionError: "Error during decryption. Check the console for details.",
            ...
            ua: {
                generatingKeys: "Генерування ключів, зачекайте... (це може зайняти деякий час)",
                keyGenError: "Помилка під час генерації ключів. Перевірте консоль для деталей.",
                enterTextEncrypt: "Будь ласка, введіть текст для шифрування!",
                enterPublicKey: "Будь ласка, введіть публічний ключ для шифрування!",
                encryptionError: "Помилка під час шифрування. Перевірте консоль для деталей.",
                enterEncryptedText: "Будь ласка, введіть зашифрований текст для дешифрування!",
                enterPrivateKey: "Будь ласка, введіть приватний ключ для дешифрування!",
                decryptionError: "Помилка під час дешифрування. Перевірте консоль для деталей.",
                publicKeyCopied: "Публічний ключ скопійовано до буфера обміну!",
    ...
    generateKeyPair() {
        ...
        setTimeout(() => {
            try {
                ...
                alert(this.getMessage('keyGenError'));
                this.showGeneratingMessage(false);
            }
        }, 100);
    }
    ...
    showGeneratingMessage(show) {
        ...
        if (!message && show) {
            ...
            message.textContent = this.getMessage('generatingKeys');
    ...
    encryptText() {
        ...
        if (!inputText) {
            return alert(this.getMessage('enterTextEncrypt'));
        }
        
        if (!publicKeyPem) {
            return alert(this.getMessage('enterPublicKey'));
        }
        ...
        try {
            ...
            alert(this.getMessage('encryptionError'));
        }
    }
    ...
    decryptText() {
        ...
        if (!encryptedInput) {
            return alert(this.getMessage('enterEncryptedText'));
        }
        
        if (!privateKeyPem) {
            return alert(this.getMessage('enterPrivateKey'));
        }
        ...
        try {
            ...
            alert(this.getMessage('decryptionError'));
        }
    }
    ...
    copyPublicKey() {
        ...
        if (publicKey) {
            ...
            alert(this.getMessage('publicKeyCopied'));
        }
    }
    ...
    copyPrivateKey() {
        ...
        if (privateKey) {
            ...
            alert(this.getMessage('privateKeyCopied'));
        }
    }
    ...
    downloadPublicKey() {
        const publicKey = document.getElementById('publicKeyDisplay').value;
        if (publicKey) {
            this.downloadFile(publicKey, 'public_key.txt');
        }
    }
    ...
    downloadPrivateKey() {
        const privateKey = document.getElementById('privateKeyDisplay').value;
        if (privateKey) {
            this.downloadFile(privateKey, 'private_key.txt');
        }
    }
